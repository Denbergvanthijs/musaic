...
from v9.Data.DataGeneratorsLead import RhythmGenerator as RhythmGeneratorBase
        ...
        for instrument_ls in song_iter:
            ...
            for rhythms, meta in instrument_ls:
                ...
                if with_metaData:
                    ...
                    prev_meta = np.vstack([np.zeros_like(prepared_meta[0]), prepared_meta[:-1]])
        ...
        for instrument_ls in song_iter:
            ...
            for melodies, meta in instrument_ls:
                ...
                if with_metaData:
                    prepared_meta = np.array(list(map(self.prepare_metaData, meta)))
                    prev_meta = np.vstack([np.zeros_like(prepared_meta[0]), prepared_meta[:-1]])
                    yield ([contexts, prepared_meta, prev_meta, lead_mat],
                           melodies_y)
                else:
